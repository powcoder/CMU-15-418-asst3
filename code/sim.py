...
class Rat:
    ...
    def next(self, loadFactor = 1.0):
        ...
        optVals = [nid.neighborILF() for nid in self.node.region]
        idx = rutil.chooseMove(self.rng, loads, optVals)
...
class Node:
    ...
    # Recompute ILF based on relative imbalance with neighbors
    def neighborILF(self):
        sum = 0.0
        count = 0
        for nd in self.region[1:]:
            r = rutil.imbalance(self.ratCount, nd.ratCount)
            sum += r
            count += 1
        ilf = 0.5 * (sum/count) + 1.75
        return ilf
...
class Simulator:
    ...
    batchFraction = 0.02 # Maximum fraction of rats in single batch
    batchSize = 0     # Number of rats in single batch

    def __init__(self, graph):
        self.nodes = [Node(i, graph.nodeList[i]) for i in xrange(len(graph.nodeList))]
    ...
    def loadRats(self, fname = "", seed = rutil.DEFAULTSEED):
        ...
        self.batchSize = max(int(math.sqrt(rcount)), int(self.batchFraction * rcount))
        return True
